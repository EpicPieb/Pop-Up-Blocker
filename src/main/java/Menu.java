...
public class Menu {
    ...
    public void GUI(Menu menu){
        ...
        try{
            ...
            ImageIcon icon = new ImageIcon(Objects.requireNonNull(classLoader.getResource("assets/icon.png")));
            ...
            { // second panel
                ...
                ImageIcon piebGif = new ImageIcon(Objects.requireNonNull(classLoader.getResource("assets/textures/pieb.gif")));
                JLabel pieb = new JLabel(piebGif);
                pieb.setBounds(385, 50, piebGif.getIconWidth(), piebGif.getIconHeight());

                ImageIcon tobyGif = new ImageIcon(Objects.requireNonNull(classLoader.getResource("assets/textures/toby.gif")));
                JLabel toby = new JLabel(tobyGif);
                toby.setBounds(450, 175, toby.getIcon().getIconWidth(), toby.getIcon().getIconHeight());

                ImageIcon chatgptGif = new ImageIcon(Objects.requireNonNull(classLoader.getResource("assets/textures/chatgpt.gif")));
            ...
            try{
                InputStream inputStream = getClass().getResourceAsStream("/assets/sounds/sans.wav");
                ...
                // i cna't use this :(
                // AudioPlayer.setAudio(inputStream);
    ...
    private void highSLabel() {
        ...
        if(DataStuff.loadedHighS != null) {
            switch (time) {
                case 5 -> {
                    switch (diff) {
                        case 1 -> {
                            if(DataStuff.loadedHighS[0] != null)
                            {highs = DataStuff.loadedHighS[0];}
                        }
                        case 2 -> {
                            if(DataStuff.loadedHighS[1] != null)
                ...
                case 60 -> {
                    switch (diff) {
                        ...
                        case 3 -> {
                            if(DataStuff.loadedHighS[18] != null)
                            {highs = DataStuff.loadedHighS[18];}
                        }
                        case 4 -> {
                            if(DataStuff.loadedHighS[19] != null)
                            {highs = DataStuff.loadedHighS[19];}
                        }
                    }
                }
